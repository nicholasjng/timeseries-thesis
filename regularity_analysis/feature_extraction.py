...
from utils.time_utils import construct_binary_visit_series, get_periodogram, \
    get_welch_periodogram, get_lombscargle_periodogram
...
def get_time_of_day_visits(customer_data, login_data_sorted, verbose=False):
    ...
    Assumption 2: IMPORTANT: The input data frame is assumed to be sorted to
    not have to deal with complex lookup by
    matching of customer codes in the customer data.
    """

    feature_list = ["night_visits", "morning_visits", "afternoon_visits",
                    "evening_visits"]

    # TODO: Include the option of returning a new dataframe
    #  or returning the data inplace
...
def get_quarterly_visits(customer_data, login_data_sorted, verbose=False):
    """
    Subroutine to calculate the percentage of visits during a given business
    quarter
    Assumption 1: Values of the hour parameter in the timestamp correspond to
    ...
    feature_list = ["1st_quarter_visits", "2nd_quarter_visits",
                    "3rd_quarter_visits", "4th_quarter_visits"]
    ...
    while index < num_logins:
        ...
        customer_data.loc[customer, feature_list] = quarterly_visits(
            customer_logins)
...
def get_seasonal_visits(customer_data, login_data_sorted, verbose=False):
    ...
    feature_list = ["winter_visits", "spring_visits", "summer_visits",
                    "autumn_visits"]
    ...
    while index < num_logins:
        ...
        customer_data.loc[customer, feature_list] = seasonal_visits(
            customer_logins)
...
def get_visit_entropies(customer_data, login_data_sorted, m=2, r=0.2,
                        int_order=4, verbose=False):
    """
    Subroutine to calculate the approximate entropy for a visit series. 
    customer_data: Copy of the customer registration database. Assumed to have
    a visit_numbers column giving the number of visits in the data
    login_data_sorted: Data frame of the login data, assumed sorted by
     customers.
    m: Window length, must be a positive integer
    r: Noise level, default set at 0.2 * series standard_deviation
    int_order: Order of integer for integer representation entropy, see
     implementation below
    """

    feature_list = ["apen_visits_m={}_r={}".format(m, r),
                    "sampen_visits_m={}_r={}".format(m, r),
                    "intrepen_n={}".format(int_order)]
    ...
    while index < num_logins:
        ...
        # TODO: Make an option to select active range
        #  instead of contract duration
        last_log_date = login_data_sorted["DATE_SAVED"].max()

        # after this, visit_series contains integers
        binary_visit_ts = construct_binary_visit_series(customer,
                                                        customer_logins,
                                                        last_log_date)
        ...
        visit_intrepen = integer_representation_entropy(binary_visit_ts,
                                                        int_order)
...
def time_of_day_visits(customer_logins):
    """
    Small subroutine to return time of day visits from an amount of
    customer login data.
    """
    visits = np.zeros(4)
    ...
    # 6 is the size of a part of day, integer division gives a number
    # between 0 and 3
    visit_daytimes = np.array(
        [visit_date.hour for visit_date in customer_logins.DATE_SAVED]) // 6
...
def seasonal_visits(customer_logins):
    """
    Small subroutine to return seasonal visits from an amount of customer
    login data.
    """
    visits = np.zeros(4)
    ...
    # 6 is the size of a part of day, integer division gives a
    # number between 0 and 3
    visit_seasonals = (np.array([visit_date.month for visit_date in
                                 customer_logins.DATE_SAVED]) % 12) // 3
...
def quarterly_visits(customer_logins):
    """
    Small subroutine to return quarterly visits from an amount of
    customer login data.
    """
    visits = np.zeros(4)
    ...
    # 6 is the size of a part of day, integer division gives a
    # number between 0 and 3
    visit_quarterlies = (np.array([visit_date.month for visit_date in
                                   customer_logins.DATE_SAVED]) - 1) // 3
...
def approximate_entropy(timeseries, m=2, r=0.2, detrend=True,
                        sigma_units=True):
    """
    Wrapper for the cythonized version to calculate the approximate entropy,
    as first introduced by Pincus et al, A Regularity Statistic for
    Medical Data Analysis, 1991

    timeseries: pd.Series of timeseries values, has to be convertible to float
    m: Window size
    r: Noise level
    detrend: Whether or not to normalize time series to mean zero and
    unit variance.
    """
    ...
    # standard deviation is saved separately because it is used to
    # calculate the noise level parameter for ApEn
...
def sample_entropy(timeseries, m=2, r=0.2, detrend=True, sigma_units=True):
    ...
    as first introduced by Richman and Moorman, Physiological time-series
    ...
    # standard deviation is saved separately because it is used to calculate
    # the noise level parameter for ApEn
    sigma = timeseries.std()
...
# TODO: Research possibilities to cythonize the
#  "getting the unique permutations" part
def permutation_entropy(timeseries, n=5, lag=1, normalize=True):
    ...
    as given in Bandt, Pompe: Permutation entropy — a natural complexity
    measure for time series (2002)

    n: Permutation order, commonly assumed between 3 and 7 
    lag: Lag parameter, determines the sliding of the computational window.
    Default is 1
    """

    N = len(timeseries.index)

    # for permutation orders larger than the series length,
    # permutation entropy is undefined
    if n > N:
        return np.nan
    ...
    # thanks to the upper bound H(n) < log(n!) where H(n) is the
    # permutation entropy of order n
    ...
    # this function returns for a given array the indices that would sort
    # this array ascending by value.
    # axis=1 specifies that each row, i.e. all the windows
    # of the time series should be sorted.
    permutations = np.argsort(perms, axis=1)

    # gives the amount of unique occurrences of each permutation
    # present in the data.
...
def integer_representation_entropy(timeseries, n=5, lag=1, normalize=True):
    ...
    # we divide by log(n!) = lngamma(n+1) to normalize everything to 1
    # thanks to the upper bound H(n) < log(n!),
    # where H(n) is the permutation entropy of order n
    ...
    # gives the amount of unique occurrences of each integer
    # present in the data.
    ...
    # thanks to the upper bound H(n) < log_2(2^n) = n,
    # where H(n) is the integer representation entropy of order n
...
def lz_complexity(timeseries):
    ...
    as first introduced by Lempel and Ziv,
    On the Complexity of Finite Sequences, 1976
...
def fourier_spectrum_entropy(spectrum, cutoff=None, normalize=True):
    """
    Entropy of a power spectrum obtained through a periodogram routine
     from scipy.signal.
    Parameters:
    spectrum: pd.Series, nonnegative entries. Holds spectrum
    cutoff: float, cutoff frequency. Used to cut off a spectrum 
    normalize: bool, whether to normalize by the entropy of the discrete
    uniform (discrete maximum entropy distribution).
    """
    try:
        ...
        spectrum_entropy = np.dot(discrete_dist_vals,
                                  np.log(discrete_dist_vals)) / np.log(2.0)

        if isclose(spectrum_entropy, 0.0):
            # if we end up here, entropy is close to zero,
...
def classical_periodogram_entropy(timeseries, cutoff=None, normalize=True,
                                  p_config=None):
    """
    Helper routine for calculating the periodogram
    and then its entropy for a time series.
    Produces easier to read code in case the periodogram
...
def welch_entropy(timeseries, cutoff=None, normalize=True, p_config=None):
    """
    Helper routine for calculating the Welch periodogram and then its entropy
    for a time series.
    Produces easier to read code in case the periodogram will not be used for
    anything else.
    """
    if p_config is None:
        raise ValueError("Something went wrong")
...
def shannon_markov_entropy(timeseries, normalize=True):
    ...
    cond_entropy = conditional_probs[nonzero] * np.log2(
        conditional_probs[nonzero])
...
def multiscale_entropy(timeseries, stat_name, scales, **kwds):
    ...
    Idea from Costa, Goldberger and Peng, "Multiscale Entropy Analysis of
    Complex Physiologic Time Series", Phys. Rev. Lett. 89, No.6, July 2002
    timeseries: pd.Series, data on which to perform the analysis
    stat_name: Has to be "apen", "sampen", "permen", "intrepen",
     entropy measure to investigate
    ...
    Returns: pd.Series of entropy corresponding to different scaling factors.
    """
    entropy_vals = np.full(len(scales), np.nan)

    stats = {"apen": approximate_entropy, "sampen": sample_entropy,
             "permen": permutation_entropy, "fse": fourier_spectrum_entropy}

    stat_key = stat_name.lower()

    # print separate error message for clarity,
    # would otherwise be caught be the try-except below
    if stat_key not in list(stats.keys()):
        raise ValueError(
            "Error: Statistic name not recognized. "
            "Available options are: \"apen\", \"sampen\", \"permen\", \"fse\"")

    for i, scale in enumerate(scales):
        try:
            resampled_series = timeseries.resample(scale).mean()
            entropy_vals[i] = stats.get(stat_name.lower())(resampled_series,
                                                           **kwds)
...
def sliding_window_calculation(timeseries, stat_name, window_size=30):
    ...
    """
    funcs = {"apen": approximate_entropy,
             "sampen": sample_entropy,
             "intrepen": integer_representation_entropy,
...
def stat_evolution_calculation(timeseries, stat_name):
    ...
    """
    funcs = {"apen": approximate_entropy, "sampen": sample_entropy,
             "intrepen": integer_representation_entropy,
             "permen": permutation_entropy, "lz_complexity": lz_complexity}
