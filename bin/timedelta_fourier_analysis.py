...
from utils.stat_utils import calculate_td_fourier_stats, get_stat_names
...
def calculate_binary_fourier_regularity(config_file, verbose=2):
    ...
    total_fourier_stats = len(experiments)
    ...
    results = login_dataframe.groupby("CUST_CODE").parallel_apply(calculate_td_fourier_stats, **kwds)
    end = time.time()
    ...
    filename = output_dir + "td_fourier_results" + data_ext
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file", default="cfg/timedelta_fourier.config.yml", help="yaml config file for statistical calculation")
