...
from utils.stat_utils import calculate_changepoints
from utils.time_utils import get_timedeltas
from utils.data_utils import *

def calculate_user_changepoints(config_file, verbose=2):
    """
    ...
    model = config["model"]
    algos = ["PELT", "BinSeg"]
    errors = ["Hausdorff", "Hamming"]
    ics = config["penalties"]
    
    full_stat_list = algos + errors
    stat_types = [stat + "_" + ic for ic in ics for stat in full_stat_list]
    ...
    results = login_dataframe.groupby(by="CUST_CODE").parallel_apply(calculate_changepoints, **kwds)
    end = time.time()
    ...
    filename = output_dir + "changepoint_results" + "_" + model + data_ext
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file", default="cfg/changepoints.config.yml", help="yaml config file for statistical calculation")
    ...
    calculate_user_changepoints(args.config_file, args.verbose)